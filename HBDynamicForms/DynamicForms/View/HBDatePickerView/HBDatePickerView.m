...
- (NSInteger)getWeekIndexInYearWithDate:(NSDate *)date
{
    ...
    NSDateComponents *comp = [calendar components:NSCalendarUnitWeekday| NSCalendarUnitWeekOfYear fromDate:date];
    
    //date:2016-05-18 09:23:51 +0000   ->  第21周
    NSInteger week = [comp weekOfYear];   //21
    
    NSLog(@"第%ld周",(long)week);
...
- (void)getNowDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    switch (_type) {
        ...
        {
            // 无第0周,所以下标-1
            indexArray = @[@(_yearIndex),@(_weekIndex - 1)];
...
- (IBAction)sureBtnAction:(id)sender {
    ...
    if (_type == YearMonthDayType)
    {
        ...
        if(_callBackDatesBlock)
        {
            _callBackDatesBlock(YearMonthDayType, [formatter stringFromDate:_scrollToDate]);
        }
    }
    else if (_type == WeekOfYearType)
    {
        ...
        //回调 第几周 _weekIndex
        [self.delegate getSelectWeeksIndex:_weekIndex];
        
        if(_callBackDatesBlock)
        {
            _callBackDatesBlock(WeekOfYearType, @(_weekIndex));
        }
    }
    else
    {
        ...
        if(_callBackDatesBlock)
        {
            _callBackDatesBlock(YearMonthAtoZType, @[[formatter stringFromDate:_startDate],[formatter stringFromDate:_endDate]]);
        }
    }
